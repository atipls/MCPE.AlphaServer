...
namespace MCPE.AlphaServer {
    public class Server {
        ...
        public Dictionary<IPEndPoint, RakNetConnection> Clients { get; private set; }
        ...
        public async Task Update() {
            ...
            var parsed = UnconnectedPacket.Parse(result.Buffer);
            var endPoint = result.RemoteEndPoint;

//            Console.WriteLine($"{parsed}");

            switch (parsed.Reliability) {
                case PacketType.UnconnectedPing: {
                    await SendRaw(endPoint,
                        UnconnectedPongPacket.FromPing(parsed.Get<UnconnectedPingPacket>(), Guid, "MCPE.AlphaServer"));
                    break;
                }
                case PacketType.OpenConnectionRequest1: {
                    await SendRaw(endPoint,
                        OpenConnectionReplyPacket.FromRequest(parsed.Get<OpenConnectionRequestPacket>(), Guid,
                            endPoint));
                    break;
                }
                ...
                case PacketType.RakNetPacket: {
                    await HandleRakNetPacket(parsed.Get<RakNetPacket>(), endPoint);
                    break;
                }
                default:
                    break;
        ...
        async Task HandleRakNetPacket(RakNetPacket rakPacket, IPEndPoint endpoint) {
            ...
            foreach (var enclosing in rakPacket.Enclosing) {
                switch (enclosing.MessageID) {
                    ...
                    case RakPacketType.ConnectedPing: {
                        await Send(Client,
                            ConnectedPongPacket.FromPing(enclosing.Get<ConnectedPingPacket>(), StartTime));
                        break;
                    }
                    ...
                    case RakPacketType.LoginRequest: {
                        ...
                        if (Client.Player.Username == "server" || // Don't impersonate the server.
                            World.GetPlayerByName(request.Username) != null) {
                            // Don't log in if there's a player already in game.
                        ...
                        await Send(Client, LoginResponsePacket.FromRequest(request, status),
                            status == Status.VersionsMatch
                                ? new StartGamePacket(request.ReliableNum.IntValue, Client.Player.EID)
                                : null
                        );
                        ...
                        foreach (var player in World.Players) {
                            Console.WriteLine(
                                $"{{ EID: {player.Player.EID} CID: {player.Player.CID} Name: {player.Player.Username} }}");
                        }
                    ...
                    case RakPacketType.MovePlayer: {
                        ...
                        Console.WriteLine(
                            $"MovePlayer: {{ ID: {packet.ID} (== {Client.Player.EID} OR == {Client.Player.CID}) }}");
        ...
        public async Task SendRaw(IPEndPoint endPoint, UnconnectedPacket packet) {
        ...
        public async Task BroadcastMessage(string message) =>
            await SendToEveryone(new MessagePacket("server", message));

        public void ListenerThread() {
            while (true) {
                Task.Run(Update).GetAwaiter().GetResult();
            }
        }
