...
namespace fNbt {
    ...
    public sealed class NbtWriter {
        ...
        /// <exception cref="Exception"> No more tags can be written -OR-
        ...
        /// <exception cref="Exception"> Not currently in a compound. </exception>
        public void EndCompound() {
            if (IsDone || parentType != NbtTagType.Compound) {
                throw new Exception("Not currently in a compound.");
            }
            GoUp();
        ...
        /// <exception cref="Exception"> Not currently in a list -OR-
        /// not all list elements have been written yet. </exception>
        public void EndList() {
            if (parentType != NbtTagType.List || IsDone) {
                throw new Exception("Not currently in a list.");
            } else if (listIndex < listSize) {
                throw new Exception("Cannot end list: not all list elements have been written yet. " +
        ...
        /// <exception cref="Exception"> No more tags can be written -OR- given tag is unacceptable at this time. </exception>
        ...
        /// <exception cref="Exception"> Not all tags have been closed yet. </exception>
        public void Finish() {
            if (!IsDone) {
                throw new Exception("Cannot finish: not all tags have been closed yet.");
            }
        }
        ...
        void EnforceConstraints(String name, NbtTagType desiredType) {
            if (IsDone) {
                throw new Exception("Cannot write any more tags: root tag has been closed.");
            }
            if (parentType == NbtTagType.List) {
                if (name != null) {
                    throw new Exception("Expecting an unnamed tag.");
                } else if (listType != desiredType) {
                    throw new Exception("Unexpected tag type (expected: " + listType + ", given: " +
                                                 desiredType);
                } else if (listIndex >= listSize) {
                    throw new Exception("Given list size exceeded.");
                }
                listIndex++;
            } else if (name == null) {
                throw new Exception("Expecting a named tag.");
            }
        }
