...
namespace MCPE.AlphaServer.Packets {
    public class RakNetPacket : Packet {
        ...
        public override string ToString() {
            if ((Flags & IS_ACK) != 0)
                return
                    $"RakNetPacket ACK: {{ Count: {Count}, MinEqualsMax: {MinEqualsMax}, PacketMin: {PacketMin}, PacketMax: {PacketMax} }}";
            if ((Flags & IS_NAK) != 0)
                return
                    $"RakNetPacket NAK: {{ Count: {Count}, MinEqualsMax: {MinEqualsMax}, PacketMin: {PacketMin}, PacketMax: {PacketMax} }}";
